...
function TermsDialog({ onClose }: { onClose: () => void }) {
  return <div className="terms-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="terms-dialog" role="dialog" aria-modal="true" aria-labelledby="terms-title" onMouseDown={(event) => event.stopPropagation()}>
      <button type="button" className="terms-close" onClick={onClose} aria-label="Close Terms and Conditions">×</button>
      <span className="eyebrow">QuestKarte legal</span>
      <h2 id="terms-title">Terms and Conditions</h2>
      <p className="terms-updated">Effective 18 July 2026 · Version 1.0</p>
      <div className="terms-content">
        <h3>1. Honest membership</h3><p>Use your own identity and provide accurate profile and task details. Do not impersonate another person, submit false documents, or use QuestKarte for unlawful, harmful, or deceptive activity.</p>
        <h3>2. Verification and private documents</h3><p>Verification documents and face photos are stored privately and reviewed by authorised QuestKarte staff. Face verification is a manual identity comparison; QuestKarte does not make an automated biometric decision. Approval is not guaranteed and staff may request clearer information.</p>
        <h3>3. Marketplace conduct</h3><p>Members must communicate respectfully, keep task details accurate, and never request unsafe, unlawful, or off-platform payment arrangements. Report suspicious content through the platform.</p>
        <h3>4. Personal responsibility</h3><p>QuestKarte helps members discover and manage local opportunities. Members remain responsible for assessing safety, agreeing scope and payment, and protecting their personal information during each arrangement.</p>
        <h3>5. Moderation</h3><p>QuestKarte may review, reject, suspend, remove, or escalate content and accounts that breach these terms or create a safety risk. You may contact support to ask about a moderation outcome.</p>
      </div>
      <button type="button" className="btn primary" onClick={onClose}>I understand</button>
    </section>
  </div>;
}
...
export default function VerificationGate({ session, status, termsAcceptedAt, onSignOut }: { session: Session; status: VerificationStatus; termsAcceptedAt: string | null; onSignOut: () => void }) {
  ...
  return <main className="verification-page"><section className="verification-card">
    <div className="verification-brand"><img src="/questkarte-logo.png" alt="QuestKarte" /><div><strong>QuestKarte</strong><span>Secure member verification</span></div></div>
    {isWaiting ? <div className="verification-waiting"><span className="waiting-icon">✓</span><span className="eyebrow">Verification received</span><h1>Your account is being reviewed.</h1><p>Your documents are private. A moderator must approve them before you can use the marketplace, post tasks, or contact members.</p><div className="review-steps"><span>Submitted</span><span className="active">Staff review</span><span>Account opened</span></div><button type="button" className="btn secondary" onClick={onSignOut}>Use a different account</button></div> : <>
