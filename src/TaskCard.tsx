  ...
  return <>
    <article className={`quest-card ${quest.tone || "rare"}`}>
      <div className="qc-thumb">
        ...
        <div className="qc-badges"><div className="qc-badge-row"><span className={`badge ${quest.category === "Academic" ? "cat-academic" : "cat-general"}`}>{quest.category}</span>{quest.kind === "swap" && <span className="badge swap">Service swap</span>}{quest.kind === "student" && <span className="badge student-only">Students only</span>}</div><button type="button" className={`qc-bookmark ${saved ? <svg viewBox="0 0 24 24" fill="#f0c05a" stroke="#f0c05a" strokeWidth="2" style={{width: 18, height: 18}}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg> : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{width: 18, height: 18}}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>}`} onClick={onSave} aria-label="Save task">{saved ? "★" : "☆"}</button></div>
