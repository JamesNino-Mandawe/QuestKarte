...
export default function TaskCard({ quest, saved, applied, isOwner, onSave, onApply, onProfileClick }: Props) {
  ...
  const openGallery = (index = 0) => { setActiveImage(index); setShowDetails(true); };
  ...
  return <>
    <article className={`quest-card ${quest.tone || "rare"}`}>
      <div className="qc-thumb">
        {allImages.length > 0 && <div className={`qc-gallery qc-gallery-${Math.min(allImages.length, 4)}`}>{visibleImages.map((src, index) => <button type="button" className="qc-gallery-photo" key={`${src}-${index}`} onClick={() => openGallery(index)} aria-label={`View photo ${index + 1} for ${quest.title}`}><img src={src} alt={`Task reference ${index + 1} for ${quest.title}`} />{index === visibleImages.length - 1 && remainingImages > 0 && <span className="qc-more-photos">+{remainingImages}</span>}</button>)}</div>}
    ...
    {showDetails && <div className="task-modal-backdrop" role="presentation" onMouseDown={() => setShowDetails(false)}><section className="task-modal" role="dialog" aria-modal="true" aria-label={`Task details: ${quest.title}`} onMouseDown={(event) => event.stopPropagation()}><button type="button" className="task-modal-close" onClick={() => setShowDetails(false)} aria-label="Close task details">×</button><div className="task-modal-photo">{allImages.length > 0 && <><img src={allImages[activeImage]} alt={`Reference photo ${activeImage + 1} for ${quest.title}`} />{allImages.length > 1 && <><button type="button" className="gallery-arrow gallery-prev" onClick={() => setActiveImage((activeImage - 1 + allImages.length) % allImages.length)} aria-label="Previous photo">‹</button><button type="button" className="gallery-arrow gallery-next" onClick={() => setActiveImage((activeImage + 1) % allImages.length)} aria-label="Next photo">›</button><div className="gallery-count">{activeImage + 1} / {allImages.length}</div></>}</>}</div><div className="task-modal-content"><div className="modal-kicker"><span className="badge cat-general">{quest.category}</span>{quest.matchPercent && <span>{quest.matchPercent}% match</span>}</div><h2>{quest.title}</h2><p className="modal-description">{quest.description}</p>{allImages.length > 1 && <div className="gallery-thumbs">{allImages.map((src, index) => <button type="button" key={`${src}-${index}`} className={index === activeImage ? "active" : ""} onClick={() => setActiveImage(index)}><img src={src} alt={`Open photo ${index + 1}`} /></button>)}</div>}<div className="modal-meta"><span>⌖ {quest.location}</span><span>◷ {quest.schedule}</span><strong>{quest.commission}</strong></div><div className="requirements"><h3>What needs to be done</h3><ul>{requirements.map((item) => <li key={item}>✓ {item}</li>)}</ul></div><div className="modal-footer"><ProfileLink quest={quest} onProfileClick={onProfileClick} prefix="Posted by " />{isOwner ? <span className="my-task-chip">My task</span> : onApply && <button type="button" className="apply-btn modal-apply" onClick={() => { onApply(); setShowDetails(false); }} disabled={applied}>{applied ? "Applied" : "Apply to this task"}</button>}</div></div></section></div>}
  </>;
}
